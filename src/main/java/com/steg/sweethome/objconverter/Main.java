package com.steg.sweethome.objconverter;
...
public class Main {
	public static void main(String[] args) throws RecorderException {
		if (args.length != 2) {
			System.out.println("Usage: Main input(.sh3d) output-directory");
			System.exit(1);
		}

		HomeFileRecorder homeFileRecorder = new HomeFileRecorder();
		Home home = homeFileRecorder.readHome(args[0]);
		
		String directory = args[1];
		new File(directory).mkdirs();
		
		exportHomeToFile(home, new File(directory, "output").getAbsolutePath(), "header", true, new Object3DBranchFactory());
	}
	...
	public static void exportHomeToFile(Home home, String objFile, String header, boolean exportAllToOBJ,
		...
		try {
			...
			for (Selectable item : exportedItems) {
				// Create a not alive new node to be able to explore its
				// coordinates without setting capabilities
